...
#import "XQCountDownButton.h"
#import "NSTimer+XQBlockSupport.h"

@interface XQCountDownButton ()
...
@implementation XQCountDownButton {
    /** 倒计时开始值 */
...
- (void)setDataSource:(id<XQCountDownButtonDataSource>)dataSource {
    if (_useBlock) {
        [self p_showError];
        return;
    }
...
- (void)setDelegate:(id<XQCountDownButtonDelegate>)delegate {
    if (_useBlock) {
        [self p_showError];
        return;
    }
...
/** 按钮点击 */
- (void)p_buttonClicked:(XQCountDownButton *)sender {
    sender.enabled = NO;
